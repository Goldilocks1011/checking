...
async def upload_fno(
    ...
    try:
        ...
        finish_task(user_id, "upload_fno")
        return result

    except Exception as e:
        logger.error(f"[UploadFNO] failed for user {user_id}: {e}", exc_info=True)
        finish_task(user_id, "upload_fno", error=str(e))
        return {"status": "error", "message": f"F&O upload failed: {e}"}