...
def fetch_spot_price(underlying: str) -> float:
    ...
    # ── 5paisa (preferred) ──────────────────────────────────────────────────
    try:
        from backend.services.engine_price_fetch import fetch_prices_with_change
    ...
    try:
        ...
        for m in merged.values():
            ...
            if net_qty > 0:
                ...
                avg_price = (
                    m["sell_value"] / m["sell_qty"] if m["sell_qty"] > 0 else 0.0
                )
            result.append(
                {
...
def apply_adjustment(user_id: int, adjustment_id: int) -> dict:
    ...
    try:
        ...
        for p in positions:
            if (
                p["underlying"] == adj.underlying
                and p["instrument_type"] == adj.instrument_type
                and abs(p["strike_price"] - float(adj.old_strike)) < 1.0
            ):
