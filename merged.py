...
def merged_transactions(user_ids: List[int] = Query(...), db: Session = Depends(get_db)):
...
def merged_pnl(user_ids: List[int] = Query(...), db: Session = Depends(get_db)):
    placeholders = ",".join(str(uid) for uid in user_ids)
    rows = db.execute(
        text(f"""
...
def merged_intraday(user_ids: List[int] = Query(...), db: Session = Depends(get_db)):
    placeholders = ",".join(str(uid) for uid in user_ids)
    rows = db.execute(
        text(f"""
...
def merged_fno_positions(user_ids: List[int] = Query(...), db: Session = Depends(get_db)):
    placeholders = ",".join(str(uid) for uid in user_ids)
    rows = db.execute(
        text(f"""
...
def merged_fno_pnl(user_ids: List[int] = Query(...), db: Session = Depends(get_db)):
    placeholders = ",".join(str(uid) for uid in user_ids)
    rows = db.execute(
        text(f"""
