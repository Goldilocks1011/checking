...
@router.get("/auth/me")
async def get_me(account_id: int = Depends(get_current_account), db: Session = Depends(get_db)):
    def _fetch():
...
async def change_pwd(data: ChangePasswordRequest, account_id: int = Depends(get_current_account)):
    ok = await asyncio.to_thread(change_password, account_id, data.old_password, data.new_password)
    if not ok:
        raise HTTPException(status_code=400, detail="Old password is incorrect")
    return {"message": "Password updated"}

@router.put("/auth/profile")
async def update_profile(data: ProfileUpdate, account_id: int = Depends(get_current_account)):
