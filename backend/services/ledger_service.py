...
def _detect_broker(filename: str) -> str:
    ...
    if (fn.startswith('equity_currency_ledger')   # underscore variant
            ...
            or ('equity' in fn and 'ledger' in fn and 'iifl' not in fn
                and not fn.startswith('ledger'))):  # catch-all for equity ledger
        return 'IIFL'

    # Zerodha: ledger-TL7712.xlsx  /  ledgerTL7712_6.xlsx  /  ledger_<id>.xlsx
    if fn.startswith('ledger-') or fn.startswith('ledgertl') or fn.startswith('ledger_'):
        return 'Zerodha'
...
def _normalise_txn(txn: dict) -> dict:
    return {
        ...
        'particular':  str(txn.get('Particular') or txn.get('Particulars') or '').strip(),
...
def process_ledger_file(
    ...
    try:
        ...
        existing = db.query(ProcessedFile).filter_by(
            user_id=user_id, filename=filename, file_type='LEDGER'
        ).first()
        if existing:
            return {
                'status': 'skipped',
        ...
        if broker == '5paisa':
            from parsers.fivepaisa_ledger import parse as p5
            parsed = p5(buf)
        elif broker == 'IIFL':
            from parsers.iifl_ledger import parse as piifl
            parsed = piifl(buf)
        elif broker == 'Zerodha':
            from parsers.zerodha_ledger import parse as pzrd
            parsed = pzrd(buf)
        else:
        ...
        db.add(ProcessedFile(
            ...
            records_added=inserted, file_type='LEDGER'
        ))
        ...
        db.rollback()
        return {'status': 'error', 'message': f'Ledger error: {e}', 'broker_warning': ''}
    finally:
        db.close()
