...
from services.tax_harvest_service import run_harvest_multi  # we'll create this
...
def tax_harvest(user_id: int, start: str, end: str, db: Session = Depends(get_db)):
    ...
    return run_harvest_multi(accounts, start, end)

# new multi‑user endpoint
@router.post("/tax-harvest")
def tax_harvest_multi(
