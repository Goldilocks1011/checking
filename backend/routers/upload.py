...
async def upload_equity(
    ...
    # IIFL uses the same Trade Listing file for both EQ and FNO — never block it.
    # For all other brokers, the engine's own duplicate check handles this;
    # this router-level check is an early exit to avoid unnecessary processing.
    ...
    # Wrap it in a BytesIO so the engine sees .getvalue() and .name
    buf = BytesIO(file_bytes)
    buf.name = file.filename      # the engine reads .name for the filename

    # Process the file (all engine logic remains untouched)
    result = process_file(buf, user_id, broker, file_type=file_type)

    # Store the raw bytes for later download
    try:
        db.execute(
...
def get_upload_history(user_id: int, db: Session = Depends(get_db)):
    rows = db.execute(
        text("SELECT id, filename, records_added, processed_at, file_type FROM processed_files WHERE user_id = :uid ORDER BY processed_at DESC"),
        {"uid": user_id}
    ).fetchall()
...
def download_file(user_id: int, filename: str, db: Session = Depends(get_db)):
    row = db.execute(
        text("SELECT file_content FROM processed_files WHERE user_id = :uid AND filename = :fn"),
