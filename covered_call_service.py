...
from backend.services.ce_pe_service import (
    _get_atm_option_info,
    _get_two_expiry_option_info,
    _get_5paisa_client,
)
import logging
...
def _load_holdings_map(user_ids: List[int], db) -> Dict[str, dict]:
    ...
    result: Dict[str, dict] = defaultdict(
        lambda: {"qty": 0.0, "avg_buy_price": 0.0, "total_invested": 0.0, "symbol": ""}
    )
    for r in rows:
    ...
    if not rows:
        ...
        for r in rows_txn:
            ...
            avg = (
                (_safe(r.buy_val) / bq)
                if net > 0 and bq > 0
                else ((_safe(r.sell_val) / sq) if sq > 0 else 0.0)
            )
...
def get_covered_call_analysis(user_id: int) -> dict:
    ...
    client = None
    try:
        from backend.services.ce_pe_service import _get_5paisa_client
    ...
    try:
        ...
        if fno_rows_for_price:
            try:
                ...
                logger.info(
                    f"[CovCall] Live F&O prices fetched: {len(live_fno_prices)} contracts"
                )
            except Exception as e:
                logger.info(f"[CovCall] fetch_fno_prices failed: {e}")

        def _live_option_price_db(
            can: str, itype: str, expiry: str, strike: float
        ) -> float:
            """
                            ...
                            if ts:
                                ...
                                if exp_dt == target_date:  # Match expiry
                                    ...
                                    if isinstance(chain, dict):
                                        opts = chain.get("Options") or chain.get(
                                            "Data", []
                                        )
                                    ...
                                    for opt in opts:
                                        ...
                                        if (
                                            ...
                                            if ltp > 0:
                                                logger.info(
                                                    f"[CovCall] ✅ REST API LTP: ₹{ltp} for {can} {itype} {strike}"
                                                )
                                                return ltp
                                    break
            except Exception as e:
            ...
            try:
                ...
                scrip_type = {"FUT": "XX", "CE": "CE", "PE": "PE"}.get(
                    itype.upper(), itype
                )
                ...
                try:
                    ...
                    if feed_client:
                        feed = _batch_market_feed(
                            [{"Exch": "N", "ExchType": "D", "ScripData": scrip_code}]
                        )
                        ltp = _safe((feed.get(scrip_code) or {}).get("ltp", 0))
                        if ltp > 0:
                            logger.info(
                                f"[CovCall] ✅ Feed LTP via scrip_code={scrip_code}: ₹{ltp}"
                            )
                            return ltp
                finally:
                    db2.close()
        ...
        for can in sorted(all_underlyings):
            ...
            sold_ce_list = [
                p
                for p in pos_list
                if p["instrument_type"] == "CE" and p["open_qty"] < 0
            ]
            long_fut_list = [
                p
                for p in pos_list
                if p["instrument_type"] == "FUT" and p["open_qty"] > 0
            ]
            short_fut_list = [
                p
                for p in pos_list
            ...
            sold_pe_list = [
                p
                for p in pos_list
                if p["instrument_type"] == "PE" and p["open_qty"] < 0
            ]
            bought_ce_list = [
                p
                for p in pos_list
                    ...
                    covered_calls.append(
                        {
                            ...
                            "eq_invested": round(
                                _safe(h_info.get("total_invested", 0)), 2
                            ),
                            "long_fut_qty": int(
                                sum(p["open_qty"] for p in long_fut_list)
                            ),
                            "ce_strike": ce["strike"],
                            "ce_expiry": ce["expiry"],
            ...
            if (has_holding or has_long_fut) and not has_sold_ce:
                fut_qty = (
                    sum(p["open_qty"] for p in long_fut_list) if long_fut_list else 0
                )

                # For equity holding: (spot - avg_cost) × qty
                unreal_eq = (
                    round((spot - avg_price) * eq_qty, 2)
                    if (spot and avg_price and eq_qty)
                    else None
                )
                ...
                if long_fut_list:
                    ...
                    total_val = sum(
                        p["open_qty"] * p["avg_price"] for p in long_fut_list
                    )
                    total_qty = sum(p["open_qty"] for p in long_fut_list)
                    fut_avg_entry = (
                        round(total_val / total_qty, 2) if total_qty else 0.0
                    )
                    ...
                    try:
                        ...
                        if two_info:
                            ...
                            if far:
                                ...
                                suggested_ce_far_premium = _safe(
                                    far.get("ce_premium", 0)
                                )
                                suggested_ce_far_expiry = far.get(
                                    "expiry_readable", "—"
                                )
                    except Exception as e:
                        logger.debug(
                            f"[CC Analysis] Could not fetch two-expiry options for {can}: {e}"
                        )
                ...
                conditions_eval = _evaluate_sell_call_conditions(
                    ...
                    spot=spot,
                    high_52w=(
                        stock_52w_high if stock_52w_high > 0 else spot + 5000
                    ),  # Fallback
                    eq_qty=eq_qty,
                )
                ...
                uncovered.append(
                    {
                        ...
                        "reason": conditions_eval[
                            "note"
                        ],  # ← NEW: Dynamic based on 4 conditions
                        "confidence_score": conditions_eval[
                            "confidence_score"
                        ],  # ← NEW: 1-5 score
                        ...
                        "suggested_ce_near_premium": round(
                            suggested_ce_near_premium, 2
                        ),
                    ...
                    correction_module.append(
                        {
                            ...
                            "live_price": (
                                round(live_price, 2) if live_price > 0 else None
                            ),
                            "spot": round(spot, 2) if spot else None,
...
def get_master_reference_positions(requesting_account_id: int) -> dict:
    ...
    try:
        ...
        child_fno = (
            _load_fno_positions_map(child_user_ids, db) if child_user_ids else {}
        )
        child_holdings = (
            _load_holdings_map(child_user_ids, db) if child_user_ids else {}
        )
        ...
        def _child_status(can: str, itype: str, strike: float) -> str:
            ...
            same_type = [
                p
                for p in child_pos_list
                if p["instrument_type"] == itype and p["open_qty"] != 0
            ]
            if not same_type:
        ...
        # ── Fetch live prices ─────────────────────────────────────────────────
        all_symbols = sorted(
            set(list(master_holdings.keys()) + list(master_fno.keys()))
        )
        spot_map: Dict[str, float] = {}
        if all_symbols:
            try:
        ...
        reference_positions.sort(
            key=lambda x: status_order.get(x.get("child_status", "NEW"), 0)
        )
        ...
        return {
            ...
            "new_signals": sum(
                1 for r in reference_positions if r["child_status"] == "NEW"
            ),
        }
...
def get_account_role(account_id: int) -> str:
    ...
    try:
        row = db.execute(
            text("SELECT role FROM accounts WHERE id=:aid LIMIT 1"), {"aid": account_id}
        ).first()
        if row and row.role:
            return str(row.role)
    except Exception:
