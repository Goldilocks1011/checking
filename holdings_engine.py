...
def _resolve_isin(symbol: str, raw_isin: str = "") -> str:
    ...
    if isin and isin not in ("NAN", "NONE", ""):
        return isin
    from backend.services.isin_resolver import resolve_isin
...
def _add_lot(state, trade_date, qty, price, source):
    if qty <= 0:
        return
    state["lots"].append(
        {"date": trade_date, "qty": qty, "price": price, "source": source}
    )
    state["qty"] += qty
...
def _load_transactions(user_id: int) -> list[dict]:
    ...
    try:
        rows = db.execute(
            text("""SELECT trade_date, symbol, company_name, exchange, segment,
...
def _load_corporate_actions(user_id: int) -> list[dict]:
    ...
    try:
        rows = db.execute(
            text("""SELECT symbol, isin, company_name, action_type, ex_date,
            ...
            if atype == "SPLIT":
                ...
                if mult > 1.001:
                    ...
                    state["ca_events"].append(
                        f"SPLIT {ratio} on {ev_date} → qty ×{mult:.4g}"
                    )
            elif atype == "BONUS":
                ...
                if frac > 0:
                    ...
                    state["ca_events"].append(
                        f"BONUS {ratio} on {ev_date} → +{bonus_qty:.2f} shares @ ₹0"
                    )
            elif atype == "MERGER":
                ...
                if not target_isin:
                    state["ca_events"].append(
                        f"⚠️ MERGER on {ev_date}: target_isin not set"
                    )
                    continue
                new_per, old_per = _parse_ratio(ratio)
                if old_per <= 0 or state["qty"] < 0.01:
                    continue
                old_qty = state["qty"]
                old_value = state["invested_value"]
                new_qty = math.floor((old_qty * new_per) / old_per)
                if new_qty <= 0:
                    continue
                old_ca_events = list(state["ca_events"])
                state["qty"] = 0.0
                state["invested_value"] = 0.0
                state["lots"] = []
                new_state = _ensure(
                    portfolio, target_isin, target_symbol, ev.get("company", symbol)
                )
                ...
                cost_pct = float(
                    ev.get("cost_split_pct", details.get("cost_split_pct", 0.0))
                )
                if not target_isin:
                    state["ca_events"].append(
                        f"⚠️ DEMERGER on {ev_date}: target_isin not set"
                    )
                    continue
                new_per, old_per = _parse_ratio(ratio)
                if old_per <= 0 or state["qty"] < 0.01:
                ...
                if child_qty > 0:
                    child_state = _ensure(
                        portfolio, target_isin, target_symbol, ev.get("company", "")
                    )
                    avg_child = (
                        round(child_value / child_qty, 4) if child_qty > 0 else 0.0
                    )
                    _add_lot(
                        child_state,
                        ev_date,
                        child_qty,
                        avg_child,
                        f"DEMERGER_FROM:{symbol}({ev_date})",
                    )
                    if not child_state["first_buy_date"]:
                        child_state["first_buy_date"] = (
                            state.get("first_buy_date") or ev_date
                        )
                    child_state["ca_events"].append(
                        f"DEMERGER {ratio} on {ev_date}: {child_qty} shares from {symbol} @ ₹{avg_child:.2f} (cost ₹{child_value:,.2f})"
                    )
                state["ca_events"].append(
...
def get_ca_aware_holdings(user_id: int) -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(rows).sort_values("symbol").reset_index(drop=True)
        if rows
        else pd.DataFrame()
    )
            ...
            rows.append(
                {
                    ...
                    "Company": s["company"],
                    "ISIN": isin_display,
                    "Lot Type": lot.get("source", "BUY"),
                    "Buy Date": lot["date"],
                    "Days Held": days_held,
                    "Term": term,
                    "Qty": round(lot["qty"], 4),
                    "Avg Cost (₹)": round(lot["price"], 2),
                    "Invested (₹)": round(lot["qty"] * lot["price"], 2),
                    "Tax Rate": tax_rate,
                }
            )
    return (
        pd.DataFrame(rows).sort_values(["Symbol", "Buy Date"]).reset_index(drop=True)
        if rows
        else pd.DataFrame()
    )
    ...
    for isin_key, s in portfolio.items():
        for ev_str in s.get("ca_events", []):
            rows.append(
                {
                    "Symbol": s["symbol"],
                    "ISIN": isin_key if not isin_key.startswith("SYM:") else "",
                    "Event": ev_str,
                }
            )
