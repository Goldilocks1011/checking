...
from backend.services.tax_harvest_service import run_harvest_multi
import asyncio
...
async def tax_harvest(
    user_id: int, start: str, end: str, db: Session = Depends(get_db)
):
...
async def tax_harvest_multi(
    ...
    for uid in user_ids:
        user_row = db.execute(
            text("SELECT username FROM users WHERE id=:uid"), {"uid": uid}
        ).first()
