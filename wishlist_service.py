...
def clear_all(user_id: int) -> dict:
    ...
    try:
        result = db.execute(
            text("DELETE FROM wishlists WHERE user_id=:uid"),
            {"uid": user_id}
        )
        db.commit()
        return {"status": "cleared", "removed": result.rowcount}
...
def clear_group_all(group_id: int) -> dict:
    ...
    try:
        result = db.execute(
            text("DELETE FROM wishlists WHERE group_id=:gid"),
            {"gid": group_id}
        )
        db.commit()
        return {"status": "cleared", "removed": result.rowcount}
...
def _check_profit_condition_wishlist(held: bool, avg_buy_price: float, spot: float) -> dict:
    """
