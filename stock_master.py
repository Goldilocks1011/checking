...
def get_unmatched(user_id: int, db: Session = Depends(get_db)):
    rows = db.execute(
        text(
            "SELECT * FROM unmatched_symbols WHERE user_id=:uid AND resolved=0 ORDER BY broker, raw_symbol"
        ),
        {"uid": user_id},
    ).fetchall()
...
def link_symbol(
    user_id: int, raw_symbol: str, broker: str, isin: str, db: Session = Depends(get_db)
):
    # 1. Ensure the ISIN exists in stock_master_mapping (if not, insert minimal row)
    existing = db.execute(
        text("SELECT isin FROM stock_master_mapping WHERE isin=:isin"), {"isin": isin}
    ).first()
    if not existing:
        db.execute(
            text(
                "INSERT INTO stock_master_mapping (isin, standard_name, canonical_symbol, fno_available, lot_size) VALUES (:isin, :name, :can, 0, 0)"
            ),
    ...
    db.execute(
        text(
            "UPDATE unmatched_symbols SET resolved=1, resolved_isin=:isin WHERE user_id=:uid AND raw_symbol=:sym AND broker=:br"
        ),
...
async def refresh_all_assets(db: Session = Depends(get_db)):
    def _do_refresh():
        ...
        for r in rows:
            ...
            db.execute(
                text(
                    "UPDATE stock_master_mapping SET fno_available=:fno, lot_size=:lot, updated_at=NOW() WHERE isin=:isin"
                ),
